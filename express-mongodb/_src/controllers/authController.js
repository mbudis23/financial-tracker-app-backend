...
exports.register = async (req, res) => {
    try {
        ...
        if (existingUser) {
            logger.warn(`Attempt to register with an already registered email: ${email}`);
            return res.status(400).json({
        ...
        if (error.code === 11000) {
            logger.error(`Duplicate entry error: Email ${req.body.email} is already registered`);
            return res.status(400).json({
...
exports.login = async (req, res) => {
    try {
        ...
        res.cookie('token', token, {
            httpOnly: true, // Mencegah akses cookie melalui JavaScript di browser
